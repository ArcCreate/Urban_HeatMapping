...
must_haves:
  ...
  key_links:
    ...
    - from: "app/main.py"
      to: "app/routers/blocks.py"
      via: "app.include_router(blocks.router, prefix='/api/v1')"
      pattern: "include_router.*blocks"
---

<objective>
Implement block data endpoints: blocks filtered by tract (required query param, returns GeoJSON FeatureCollection) and single block detail. Wire the router into main.py.
...
Output: app/services/blocks.py, app/routers/blocks.py (updated app/main.py), tests/test_blocks.py
</objective>
...
<task type="auto">
  ...
  <files>app/services/blocks.py, app/routers/blocks.py, app/main.py</files>
  <action>
...
**Update `app/main.py`**: Add blocks router import and registration.
...
Find the existing router registration block and add the blocks router:
```python
...
assert '/api/v1/blocks' in paths, f'/api/v1/blocks missing from routes: {paths}'
assert '/api/v1/blocks/{block_id}' in paths, '/api/v1/blocks/{block_id} missing'
print('PASS')
" 2>&amp;1</automated>
  </verify>
  <done>Service functions import cleanly; blocks router exposes GET /blocks and GET /blocks/{block_id}; app.main registers both routes under /api/v1</done>
</task>
...
- app/main.py registers blocks router under /api/v1
