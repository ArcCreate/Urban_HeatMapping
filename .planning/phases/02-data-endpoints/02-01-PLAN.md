---
phase: 02-data-endpoints
plan: "01"
type: execute
wave: 1
depends_on: []
files_modified:
  - scripts/build_duckdb.py
  - scripts/requirements-pipeline.txt
autonomous: true
requirements:
  - TRACT-01
  - TRACT-02
  - TRACT-03
  - BLOCK-01
  - BLOCK-02
  - PRED-01
  - PRED-02
  - SUM-01
  - BATCH-01

must_haves:
  truths:
    - "king_county.duckdb exists in the project root after the script runs"
    - "king_county.duckdb contains a tract_features table with 492 rows, a geometry_wkt column (non-NULL WKT strings), and all averaged numeric feature columns"
    - "king_county.duckdb contains a tract_outputs_with_preds table with tract_id, xgb_heat_score, xgb_risk_score, tf_risk_score columns (all DOUBLE in [0,1])"
    - "king_county.duckdb contains a blocks table with 25,552 rows including block_id, tract_id, and geometry_wkt columns"
    - "Geometry is stored in WGS84 (EPSG:4326) — coordinates are in [-122.x, 47.x] range for King County"
    - "DuckDB spatial extension can execute ST_AsGeoJSON(ST_GeomFromText(geometry_wkt)) on rows from both tract_features and blocks without errors"
  artifacts:
    - path: "scripts/build_duckdb.py"
      provides: "Offline pipeline script that creates king_county.duckdb from CSVs + TIGER geometry"
      exports: []
    - path: "scripts/requirements-pipeline.txt"
      provides: "Pipeline-only dependencies (pygris, geopandas, shapely) — NOT in main requirements.txt"
    - path: "king_county.duckdb"
      provides: "DuckDB file with tract_features, tract_outputs_with_preds, blocks tables"
      contains: "tract_features, tract_outputs_with_preds, blocks"
  key_links:
    - from: "scripts/build_duckdb.py"
      to: "HeatHealthKC.csv"
      via: "read_csv_auto('HeatHealthKC.csv')"
      pattern: "read_csv_auto.*HeatHealthKC"
    - from: "scripts/build_duckdb.py"
      to: "TempKC.csv"
      via: "read_csv_auto('TempKC.csv')"
      pattern: "read_csv_auto.*TempKC"
    - from: "scripts/build_duckdb.py"
      to: "pygris.tracts"
      via: "pygris.tracts(state='WA', county='033', year=2020, cb=True)"
      pattern: "pygris.tracts"
    - from: "scripts/build_duckdb.py"
      to: "pygris.blocks"
      via: "pygris.blocks(state='WA', county='033', year=2020)"
      pattern: "pygris.blocks"
---

<objective>
Build the king_county.duckdb file from raw CSVs plus Census TIGER/Line geometry. This is the prerequisite for every data endpoint in Phase 2 — no endpoint can be tested against real data without it.

Purpose: The two CSVs (HeatHealthKC.csv, TempKC.csv) contain no geometry. The DuckDB file must be built with tract and block geometries sourced from Census TIGER/Line shapefiles via pygris, combined with the CSV data, and populated with placeholder model scores. This is a one-time offline pipeline step.

Output: scripts/build_duckdb.py, scripts/requirements-pipeline.txt, king_county.duckdb (on disk)
</objective>

<execution_context>
@/Users/yashpersonal/.claude/get-shit-done/workflows/execute-plan.md
@/Users/yashpersonal/.claude/get-shit-done/templates/summary.md
</execution_context>

<context>
@.planning/PROJECT.md
@.planning/ROADMAP.md
@.planning/phases/02-data-endpoints/02-RESEARCH.md
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create pipeline requirements and build_duckdb.py script</name>
  <files>scripts/requirements-pipeline.txt, scripts/build_duckdb.py</files>
  <action>
Create `scripts/` directory (if it does not exist) and write the following two files.

---

**`scripts/requirements-pipeline.txt`** — pipeline-only dependencies (DO NOT add to main requirements.txt):

```
# Data pipeline dependencies — offline use only
# These are NOT imported by the FastAPI application.
# Install in same conda env: pip install -r scripts/requirements-pipeline.txt

pygris>=0.1.7
geopandas>=0.14
shapely>=2.0
```

---

**`scripts/build_duckdb.py`** — the full pipeline script:

```python
#!/usr/bin/env python3
"""
Build king_county.duckdb from raw CSV sources + Census TIGER/Line geometry.

USAGE (from project root, with pipeline deps installed):
    pip install -r scripts/requirements-pipeline.txt
    python scripts/build_duckdb.py

OUTPUTS:
    king_county.duckdb  — DuckDB file with 3 tables:
        - tract_features          (492 rows, features + geometry_wkt)
        - tract_outputs_with_preds (492 rows, placeholder model scores [0,1])
        - blocks                  (25,552 rows, block attributes + geometry_wkt)

NOTE ON SCORES:
    tract_outputs_with_preds is populated with PLACEHOLDER scores
    derived from normalizing the raw PRED columns in TempKC.csv.
    Replace with real model outputs from the ML training pipeline
    (XGBoost heat, XGBoost risk, TensorFlow risk) when available.
"""

import json
import logging
import sys
from pathlib import Path

import duckdb
import geopandas as gpd
import pandas as pd
import pygris

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger(__name__)

# ── Paths ──────────────────────────────────────────────────────────────────
PROJECT_ROOT = Path(__file__).parent.parent
CSV_BLOCKS = PROJECT_ROOT / "HeatHealthKC.csv"
CSV_TRACTS = PROJECT_ROOT / "TempKC.csv"
DUCKDB_PATH = PROJECT_ROOT / "king_county.duckdb"

KING_COUNTY_STATE = "WA"
KING_COUNTY_FIPS  = "033"
TIGER_YEAR        = 2020


def build(overwrite: bool = True) -> None:
    if DUCKDB_PATH.exists() and overwrite:
        log.info("Removing existing %s", DUCKDB_PATH)
        DUCKDB_PATH.unlink()

    log.info("Connecting to %s", DUCKDB_PATH)
    con = duckdb.connect(str(DUCKDB_PATH))
    con.execute("INSTALL spatial; LOAD spatial;")

    # ── Step 1: Load raw CSVs ─────────────────────────────────────────────
    log.info("Loading HeatHealthKC.csv (%s)...", CSV_BLOCKS)
    con.execute(f"""
        CREATE TABLE blocks_raw AS
        SELECT * FROM read_csv_auto('{CSV_BLOCKS}', header=True, ignore_errors=True)
    """)
    block_count = con.execute("SELECT COUNT(*) FROM blocks_raw").fetchone()[0]
    log.info("  blocks_raw: %d rows", block_count)

    log.info("Loading TempKC.csv (%s)...", CSV_TRACTS)
    con.execute(f"""
        CREATE TABLE tract_preds_raw AS
        SELECT * FROM read_csv_auto('{CSV_TRACTS}', header=True, ignore_errors=True)
    """)
    pred_count = con.execute("SELECT COUNT(*) FROM tract_preds_raw").fetchone()[0]
    log.info("  tract_preds_raw: %d rows", pred_count)

    # ── Step 2: Aggregate blocks → tract_features ─────────────────────────
    # Join key: RIGHT(TempKC.GEO_ID, 11) = CAST(HeatHealthKC."TRACT GEO ID" AS VARCHAR)
    # This yields 492 joinable tracts.
    log.info("Aggregating blocks to tract level...")
    con.execute("""
        CREATE TABLE tract_features AS
        SELECT
            CAST(b."TRACT GEO ID" AS VARCHAR)                           AS tract_id,
            AVG(b."Median afternoon temperature")                        AS mean_afternoon_temp,
            AVG(b."Median morning temperature")                          AS mean_morning_temp,
            AVG(b."Median evening temperature")                          AS mean_evening_temp,
            AVG(b."Percent tree coverage")                               AS mean_tree_cov,
            AVG(b."Percent impervious coverage")                         AS mean_imperv,
            AVG(b."Mean distance to water")                              AS mean_dist_water,
            AVG(b."Cardiovascular disease mortality rate")               AS mean_cvd_rate,
            AVG(b."Percent adults with diabetes")                        AS mean_diabetes,
            AVG(b."Life expectancy")                                     AS mean_life_expectancy,
            AVG(b."SVI Overall percentile ranking")                      AS mean_svi_overall,
            AVG(b."Percent of population whose income in the past 12 months is under 2.00x (200% of) the federal poverty level")
                                                                         AS mean_poverty2x,
            AVG(b."Percent of population with a disability")             AS mean_disability,
            AVG(b."Percent of adults 18 years and over who have limited English ability")
                                                                         AS mean_limited_english,
            AVG(b."Percent under 18 years old")                         AS mean_under18,
            AVG(b."Percent severe housing cost burdened")               AS mean_severe_cost,
            ANY_VALUE(b.CITYNAME)                                        AS city_name,
            NULL::VARCHAR                                                AS geometry_wkt
        FROM blocks_raw b
        GROUP BY CAST(b."TRACT GEO ID" AS VARCHAR)
    """)
    tract_count = con.execute("SELECT COUNT(*) FROM tract_features").fetchone()[0]
    log.info("  tract_features: %d rows", tract_count)

    # ── Step 3: Download TIGER tract geometry via pygris ─────────────────
    log.info("Downloading Census TIGER tract geometry (state=%s, county=%s, year=%d)...",
             KING_COUNTY_STATE, KING_COUNTY_FIPS, TIGER_YEAR)
    tracts_gdf = pygris.tracts(
        state=KING_COUNTY_STATE, county=KING_COUNTY_FIPS,
        year=TIGER_YEAR, cb=True  # cb=True = cartographic boundary (simplified)
    )
    tracts_gdf = tracts_gdf.to_crs("EPSG:4326")  # GeoJSON requires WGS84 — critical
    log.info("  Downloaded %d tract geometries", len(tracts_gdf))

    # GEOID column from pygris is 11-digit string — matches tract_id in tract_features
    tract_geom_map: dict[str, str] = {
        row["GEOID"]: row["geometry"].wkt
        for _, row in tracts_gdf.iterrows()
    }

    # Merge WKT geometry back into tract_features via pandas bridge
    df_tracts = con.execute(
        "SELECT tract_id FROM tract_features"
    ).fetchdf()
    df_tracts["geometry_wkt"] = df_tracts["tract_id"].map(tract_geom_map)

    matched = df_tracts["geometry_wkt"].notna().sum()
    log.info("  Geometry matched for %d / %d tracts", matched, len(df_tracts))
    if matched < len(df_tracts) * 0.9:
        log.warning("  WARNING: fewer than 90%% of tracts matched geometry — check GEOID join keys")

    # Register the pandas DataFrame and UPDATE tract_features
    con.register("tract_geom_update", df_tracts)
    con.execute("""
        UPDATE tract_features AS tf
        SET geometry_wkt = tgu.geometry_wkt
        FROM tract_geom_update AS tgu
        WHERE tf.tract_id = tgu.tract_id
    """)
    log.info("  tract_features geometry_wkt column populated")

    # ── Step 4: Create tract_outputs_with_preds (placeholder scores) ──────
    # Scores are NORMALIZED to [0,1] range from PRED12_PE (percentage, 0–100).
    # The JOIN key: RIGHT(GEO_ID, 11) matches tract_id in tract_features.
    # Replace these placeholder scores with real XGBoost/TF model outputs from
    # the offline training pipeline when trained model files are available.
    log.info("Creating tract_outputs_with_preds with normalized placeholder scores...")
    con.execute("""
        CREATE TABLE tract_outputs_with_preds AS
        SELECT
            tf.tract_id,
            -- xgb_heat_score: derived from PRED0_PE (normalized to [0,1])
            ROUND(
                COALESCE(
                    CAST(tp.PRED0_PE AS DOUBLE) / 100.0,
                    0.5  -- fallback if join fails
                ), 4
            ) AS xgb_heat_score,
            -- xgb_risk_score: derived from PRED12_PE (normalized to [0,1])
            ROUND(
                COALESCE(
                    CAST(tp.PRED12_PE AS DOUBLE) / 100.0,
                    0.5
                ), 4
            ) AS xgb_risk_score,
            -- tf_risk_score: derived from PRED3_PE (normalized to [0,1])
            ROUND(
                COALESCE(
                    CAST(tp.PRED3_PE AS DOUBLE) / 100.0,
                    0.5
                ), 4
            ) AS tf_risk_score
        FROM tract_features tf
        LEFT JOIN tract_preds_raw tp
            ON tf.tract_id = RIGHT(tp.GEO_ID, 11)
    """)
    scores_count = con.execute("SELECT COUNT(*) FROM tract_outputs_with_preds").fetchone()[0]
    score_range = con.execute("""
        SELECT
            MIN(xgb_heat_score), MAX(xgb_heat_score),
            MIN(xgb_risk_score), MAX(xgb_risk_score)
        FROM tract_outputs_with_preds
    """).fetchone()
    log.info("  tract_outputs_with_preds: %d rows", scores_count)
    log.info(
        "  Score ranges — heat: [%.3f, %.3f], risk: [%.3f, %.3f]",
        score_range[0], score_range[1], score_range[2], score_range[3]
    )

    # ── Step 5: Download TIGER block geometry and create blocks table ──────
    log.info("Downloading Census TIGER block geometry (this may take 1–2 min, ~200MB download)...")
    blocks_gdf = pygris.blocks(
        state=KING_COUNTY_STATE, county=KING_COUNTY_FIPS,
        year=TIGER_YEAR
    )
    blocks_gdf = blocks_gdf.to_crs("EPSG:4326")  # WGS84 required for GeoJSON
    log.info("  Downloaded %d block geometries", len(blocks_gdf))

    # GEOID20 column from pygris is 15-digit string — matches "BLOCK GEO ID" in HeatHealthKC
    block_geom_map: dict[str, str] = {
        row["GEOID20"]: row["geometry"].wkt
        for _, row in blocks_gdf.iterrows()
    }

    # Join block attributes from HeatHealthKC with geometry
    df_blocks_raw = con.execute("""
        SELECT
            CAST(b."BLOCK GEO ID" AS VARCHAR)              AS block_id,
            CAST(b."TRACT GEO ID" AS VARCHAR)              AS tract_id,
            b."Median afternoon temperature"               AS mean_afternoon_temp,
            b."Median morning temperature"                 AS mean_morning_temp,
            b."Median evening temperature"                 AS mean_evening_temp,
            b."Percent tree coverage"                      AS mean_tree_cov,
            b."Percent impervious coverage"                AS mean_imperv,
            b.CITYNAME                                     AS city_name
        FROM blocks_raw b
    """).fetchdf()

    df_blocks_raw["geometry_wkt"] = df_blocks_raw["block_id"].map(block_geom_map)
    block_geom_matched = df_blocks_raw["geometry_wkt"].notna().sum()
    log.info(
        "  Block geometry matched: %d / %d",
        block_geom_matched, len(df_blocks_raw)
    )

    # Write blocks table to DuckDB
    con.register("blocks_with_geom", df_blocks_raw)
    con.execute("""
        CREATE TABLE blocks AS
        SELECT * FROM blocks_with_geom
    """)
    blocks_written = con.execute("SELECT COUNT(*) FROM blocks").fetchone()[0]
    log.info("  blocks table: %d rows", blocks_written)

    # ── Step 6: Verify and close ───────────────────────────────────────────
    log.info("Running verification queries...")

    # Check spatial extension works on real data
    sample = con.execute("""
        SELECT ST_AsGeoJSON(ST_GeomFromText(geometry_wkt))::TEXT AS geom_json
        FROM tract_features
        WHERE geometry_wkt IS NOT NULL
        LIMIT 1
    """).fetchone()
    if sample and sample[0]:
        parsed = json.loads(sample[0])
        assert parsed["type"] in ("Polygon", "MultiPolygon"), \
            f"Unexpected geometry type: {parsed['type']}"
        log.info("  Spatial extension check: OK (type=%s)", parsed["type"])
    else:
        log.warning("  No geometry found for spatial extension check — verify geometry_wkt column")

    # Summarize table counts
    tables = con.execute(
        "SELECT table_name, estimated_size FROM duckdb_tables() ORDER BY table_name"
    ).fetchall()
    log.info("Tables in %s:", DUCKDB_PATH.name)
    for table_name, size in tables:
        count = con.execute(f"SELECT COUNT(*) FROM {table_name}").fetchone()[0]
        log.info("  %-35s %6d rows", table_name, count)

    con.close()
    log.info("Build complete: %s (%.1f MB)", DUCKDB_PATH,
             DUCKDB_PATH.stat().st_size / 1_048_576)


if __name__ == "__main__":
    build()
```

IMPORTANT notes for the executor:
- Run this script from the PROJECT ROOT, not from the `scripts/` directory: `python scripts/build_duckdb.py`
- The block geometry download is ~200MB and may take 1–2 minutes.
- Both CSV files (`HeatHealthKC.csv`, `TempKC.csv`) must be present in the project root.
- `pygris`, `geopandas`, `shapely` must be installed in the same conda env as the project.
- Do NOT add pygris/geopandas/shapely to the main `requirements.txt` — they are pipeline-only.
  </action>
  <verify>
    <automated>cd "/Users/yashpersonal/Downloads/Urban HeatMapping/Urban_HeatMapping" &amp;&amp; test -f scripts/build_duckdb.py &amp;&amp; test -f scripts/requirements-pipeline.txt &amp;&amp; grep -q "pygris" scripts/requirements-pipeline.txt &amp;&amp; grep -q "pygris" scripts/build_duckdb.py &amp;&amp; echo "PASS: files created"</automated>
  </verify>
  <done>scripts/build_duckdb.py and scripts/requirements-pipeline.txt exist; build_duckdb.py references pygris, geopandas, HeatHealthKC.csv, TempKC.csv; pipeline deps are NOT in main requirements.txt</done>
</task>

<task type="auto">
  <name>Task 2: Install pipeline deps and run build_duckdb.py to produce king_county.duckdb</name>
  <files>king_county.duckdb</files>
  <action>
Install pipeline dependencies and execute the build script. Run all commands from the project root in the `urban-heatmap` conda environment (Python 3.12).

```bash
# Install pipeline-only dependencies
pip install -r scripts/requirements-pipeline.txt

# Run the build script (downloads ~200MB TIGER block data — may take 1-2 min)
python scripts/build_duckdb.py
```

Expected log output indicates:
- blocks_raw: ~25,552 rows
- tract_features: ~492 rows
- Tract geometry matched for 490+ tracts (90%+ match rate)
- tract_outputs_with_preds: 492 rows with scores in [0, 1] range
- blocks: ~25,552 rows
- Spatial extension check: OK (type=Polygon or MultiPolygon)
- Build complete: king_county.duckdb (size in MB logged)

If pygris download fails (network timeout), re-run the script — pygris caches downloaded TIGER files locally after first download.

If a `geometry_wkt IS NULL` warning appears for many tracts, check that the tract GEOID format matches. The pygris `GEOID` column is a zero-padded 11-digit string like `"53033010800"`. The `tract_id` in `tract_features` is built from `CAST("TRACT GEO ID" AS VARCHAR)` which should also produce `"53033010800"`. If there is a mismatch (e.g., leading zeros stripped), fix by zero-padding: `LPAD(CAST(...), 11, '0')` in the aggregation SQL.

After the script completes, validate manually:
```bash
python -c "
import duckdb, json
con = duckdb.connect('king_county.duckdb', read_only=True)
con.execute('LOAD spatial;')
# Check table row counts
for t in ['tract_features', 'tract_outputs_with_preds', 'blocks']:
    n = con.execute(f'SELECT COUNT(*) FROM {t}').fetchone()[0]
    print(f'{t}: {n} rows')
# Check geometry is valid WKT that converts to GeoJSON
geom = con.execute(\"\"\"
    SELECT ST_AsGeoJSON(ST_GeomFromText(geometry_wkt))::TEXT
    FROM tract_features WHERE geometry_wkt IS NOT NULL LIMIT 1
\"\"\").fetchone()[0]
parsed = json.loads(geom)
assert parsed['type'] in ('Polygon', 'MultiPolygon'), f'Bad type: {parsed[\"type\"]}'
print(f'Geometry type: {parsed[\"type\"]} — PASS')
# Check scores are in [0,1]
mn, mx = con.execute('SELECT MIN(xgb_heat_score), MAX(xgb_heat_score) FROM tract_outputs_with_preds').fetchone()
assert 0.0 <= mn and mx <= 1.0, f'Score out of range: [{mn}, {mx}]'
print(f'Score range: [{mn:.3f}, {mx:.3f}] — PASS')
con.close()
print('king_county.duckdb validation: PASS')
"
```
  </action>
  <verify>
    <automated>cd "/Users/yashpersonal/Downloads/Urban HeatMapping/Urban_HeatMapping" &amp;&amp; python -c "
import duckdb, json
con = duckdb.connect('king_county.duckdb', read_only=True)
con.execute('LOAD spatial;')
tf = con.execute('SELECT COUNT(*) FROM tract_features').fetchone()[0]
tp = con.execute('SELECT COUNT(*) FROM tract_outputs_with_preds').fetchone()[0]
bl = con.execute('SELECT COUNT(*) FROM blocks').fetchone()[0]
assert tf >= 480, f'tract_features: {tf} rows (expected ~492)'
assert tp >= 480, f'tract_outputs_with_preds: {tp} rows (expected ~492)'
assert bl >= 20000, f'blocks: {bl} rows (expected ~25552)'
geom = con.execute(\"SELECT ST_AsGeoJSON(ST_GeomFromText(geometry_wkt))::TEXT FROM tract_features WHERE geometry_wkt IS NOT NULL LIMIT 1\").fetchone()[0]
parsed = json.loads(geom)
assert parsed['type'] in ('Polygon','MultiPolygon'), f'Bad geometry type: {parsed[\"type\"]}'
mn, mx = con.execute('SELECT MIN(xgb_heat_score), MAX(xgb_heat_score) FROM tract_outputs_with_preds').fetchone()
assert 0.0 &lt;= mn and mx &lt;= 1.0, f'Scores out of [0,1]: [{mn},{mx}]'
con.close()
print('PASS')
" 2>&amp;1</automated>
  </verify>
  <done>king_county.duckdb exists and passes all validation checks: tract_features has 480+ rows with non-NULL geometry_wkt, tract_outputs_with_preds has scores in [0,1], blocks has 20,000+ rows; DuckDB spatial can convert geometry_wkt to GeoJSON</done>
</task>

</tasks>

<verification>
After both tasks:

```bash
cd "/Users/yashpersonal/Downloads/Urban HeatMapping/Urban_HeatMapping"
python -c "
import duckdb, json
con = duckdb.connect('king_county.duckdb', read_only=True)
con.execute('LOAD spatial;')

# Table counts
for t in ['tract_features', 'tract_outputs_with_preds', 'blocks']:
    n = con.execute(f'SELECT COUNT(*) FROM {t}').fetchone()[0]
    print(f'{t}: {n} rows')

# Spatial roundtrip
geom = con.execute(\"\"\"
    SELECT ST_AsGeoJSON(ST_GeomFromText(geometry_wkt))::TEXT
    FROM tract_features WHERE geometry_wkt IS NOT NULL LIMIT 1
\"\"\").fetchone()[0]
parsed = json.loads(geom)
print(f'Geometry type: {parsed[\"type\"]}')

# Score range
mn, mx = con.execute(
    'SELECT MIN(xgb_heat_score), MAX(xgb_heat_score) FROM tract_outputs_with_preds'
).fetchone()
print(f'Heat score range: [{mn:.3f}, {mx:.3f}]')
con.close()
print('Verification: PASS')
"
```

Expected: All three tables have expected row counts, geometry type is Polygon or MultiPolygon, score range is within [0, 1].
</verification>

<success_criteria>
- scripts/build_duckdb.py exists and is executable
- scripts/requirements-pipeline.txt lists pygris, geopandas, shapely (NOT in main requirements.txt)
- king_county.duckdb exists at project root
- tract_features: 480+ rows, geometry_wkt column populated for 90%+ of tracts in WGS84
- tract_outputs_with_preds: 480+ rows, xgb_heat_score/xgb_risk_score/tf_risk_score all DOUBLE in [0,1]
- blocks: 20,000+ rows with block_id, tract_id, geometry_wkt columns
- DuckDB spatial extension can run ST_AsGeoJSON(ST_GeomFromText(geometry_wkt)) on tract and block rows without error
</success_criteria>

<output>
After completion, create `.planning/phases/02-data-endpoints/02-01-SUMMARY.md` using the summary template.
</output>
