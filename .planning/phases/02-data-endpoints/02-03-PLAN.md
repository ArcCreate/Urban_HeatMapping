...
must_haves:
  ...
  key_links:
    ...
    - from: "app/main.py"
      to: "app/routers/tracts.py"
      via: "app.include_router(tracts.router, prefix='/api/v1')"
      pattern: "include_router.*tracts"
---

<objective>
Implement the tract data endpoints: all tracts as GeoJSON FeatureCollection, single tract detail, single tract geometry-only, and the batch multi-tract endpoint. Wire the router into main.py.
...
Output: app/services/tracts.py, app/routers/tracts.py (updated app/main.py), tests/test_tracts.py
</objective>
...
**Update `app/main.py`**: Add the tracts router import and registration. The existing file has a comment placeholder for tracts — replace that comment block with the active import and include_router call.
...
Also add the tracts import at the top of main.py alongside the health import:
```python
from app.routers import health, tracts
```
  </action>
  <verify>
...
assert '/api/v1/tracts' in paths, f'/api/v1/tracts not in routes: {paths}'
assert '/api/v1/tracts/{tract_id}' in paths, '/api/v1/tracts/{tract_id} missing'
assert '/api/v1/tracts/{tract_id}/geometry' in paths, '/api/v1/tracts/{tract_id}/geometry missing'
assert '/api/v1/tracts/batch' in paths, '/api/v1/tracts/batch missing'
print('PASS')
" 2>&amp;1</automated>
  </verify>
  <done>Service functions import cleanly; router exposes all 4 tract routes; app.main registers /api/v1/tracts, /api/v1/tracts/{tract_id}, /api/v1/tracts/{tract_id}/geometry, /api/v1/tracts/batch</done>
</task>
...
- app/main.py registers tracts router under /api/v1
