...
Plans:
- [x] 02-01-PLAN.md — DuckDB build pipeline: scripts/build_duckdb.py creates king_county.duckdb from CSVs + TIGER geometry
- [x] 02-02-PLAN.md — Interface contracts: all Pydantic schemas + shared pytest fixture (geojson-pydantic added to requirements.txt)
- [x] 02-03-PLAN.md — Tract endpoints: GET /tracts, /tracts/{id}, /tracts/{id}/geometry, POST /tracts/batch (TRACT-01, TRACT-02, TRACT-03, BATCH-01)
- [x] 02-04-PLAN.md — Block endpoints: GET /blocks?tract_id=, /blocks/{block_id} (BLOCK-01, BLOCK-02)
- [x] 02-05-PLAN.md — Prediction, summary endpoints: GET /predictions/tracts, /predictions/tracts/ranked, /summary/county (PRED-01, PRED-02, SUM-01)
...
Plans:
- [x] 03-01-PLAN.md — Schemas + service: app/schemas/simulations.py, app/services/simulations.py (parametric formula engine with shared _compute_simulation)
- [x] 03-02-PLAN.md — Router + tests + wiring: app/routers/simulations.py, tests/test_simulations.py, app/main.py (SIM-01, SIM-02)
...
Plans:
- [x] 04-01-PLAN.md — Schemas, service, async router, tests, and main.py wiring: app/schemas/chat.py, app/services/chat.py, app/routers/chat.py, tests/test_chat.py, app/main.py (CHAT-01)
