...
**Update `app/main.py`**: This plan is the CANONICAL main.py writer for Wave 3. Plans 02-03 and 02-04 do NOT touch main.py — all Wave 3 router wiring is concentrated here to eliminate concurrent write conflicts.

At the start of this task, main.py contains only the health router (Phase 1 state). Add all Phase 2 routers in one atomic update.

Find the existing router import line (Phase 1 state — health only):
```python
...
Find the router registration block (Phase 1 state):
```python
app.include_router(health.router, prefix="/api/v1")
# Future phases add:
# app.include_router(tracts.router, prefix="/api/v1")
```
Replace with the complete Phase 2 registration:
```python
app.include_router(health.router, prefix="/api/v1")
app.include_router(tracts.router, prefix="/api/v1")
